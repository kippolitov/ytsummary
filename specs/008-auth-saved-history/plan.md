...
| **QG-3 UX Review** | Loading indicators for > 300 ms ops; no silent frozen UI; plain-language errors | ✅ Sign-in prompt and invitation-only message are explicit UI states (FR-004); Save action and Saved-view list/restore show loading indicators consistent with existing `LoadingIndicator` patterns; failed saves surface a plain-language, non-blocking error (FR-018) |
...
│   │   └── savedVideosStore.ts             (new — SavedVideos table CRUD + chat-history chunking, data-model.md)
│   └── models/
